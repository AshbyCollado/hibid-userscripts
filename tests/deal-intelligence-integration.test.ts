...
test('Chrome and Waterfox use direct background Amazon transport without opening helper tabs', async () => {
  ...
  assert.equal(chrome.version, '0.3.33');
...
test('unpacked builds self-reload only when the installed semantic version changes', () => {
  assert.equal(shouldReloadExtension('0.3.33', '0.3.33'), false);
  assert.equal(shouldReloadExtension('0.3.33', '0.3.34'), true);
  assert.equal(shouldReloadExtension('0.3.33', 'not-a-version'), false);
});
