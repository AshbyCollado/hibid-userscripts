...
test('Chrome and Waterfox use direct background Amazon transport without opening helper tabs', async () => {
  ...
  assert.equal(chrome.version, '0.3.32');
...
test('unpacked builds self-reload only when the installed semantic version changes', () => {
  assert.equal(shouldReloadExtension('0.3.32', '0.3.32'), false);
  assert.equal(shouldReloadExtension('0.3.32', '0.3.33'), true);
  assert.equal(shouldReloadExtension('0.3.32', 'not-a-version'), false);
});
