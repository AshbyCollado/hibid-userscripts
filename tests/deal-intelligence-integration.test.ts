...
test('Chrome and Waterfox use direct background Amazon transport without opening helper tabs', async () => {
  ...
  assert.equal(chrome.version, '0.3.30');
...
test('unpacked builds self-reload only when the installed semantic version changes', () => {
  assert.equal(shouldReloadExtension('0.3.30', '0.3.30'), false);
  assert.equal(shouldReloadExtension('0.3.30', '0.3.31'), true);
  assert.equal(shouldReloadExtension('0.3.30', 'not-a-version'), false);
});
