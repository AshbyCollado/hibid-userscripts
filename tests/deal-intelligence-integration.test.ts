...
test('Chrome and Waterfox use direct background Amazon transport without opening helper tabs', async () => {
  ...
  assert.equal(chrome.version, '0.3.35');
...
test('unpacked builds self-reload only when the installed semantic version changes', () => {
  assert.equal(shouldReloadExtension('0.3.35', '0.3.35'), false);
  assert.equal(shouldReloadExtension('0.3.35', '0.3.36'), true);
  assert.equal(shouldReloadExtension('0.3.35', 'not-a-version'), false);
});
