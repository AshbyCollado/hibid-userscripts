...
test('Chrome and Waterfox use direct background Amazon transport without opening helper tabs', async () => {
  ...
  assert.equal(chrome.version, '0.3.31');
...
test('unpacked builds self-reload only when the installed semantic version changes', () => {
  assert.equal(shouldReloadExtension('0.3.31', '0.3.31'), false);
  assert.equal(shouldReloadExtension('0.3.31', '0.3.32'), true);
  assert.equal(shouldReloadExtension('0.3.31', 'not-a-version'), false);
});
