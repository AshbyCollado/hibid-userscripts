...
test('toolbar activity uses a visible spinner badge and prioritizes scraping', () => {
  ...
  const restoring = { ...analysis, phase: 'restoring', message: 'Restoring saved prices' };
  ...
  assert.match(toolbarActivityPresentation({ analysis: restoring }).title, /restoring saved prices \(12\/40\)/);
...
test('toolbar activity validates messages and busy phases', () => {
  ...
  assert.equal(isAnalysisActivityPhase('restoring'), true);
